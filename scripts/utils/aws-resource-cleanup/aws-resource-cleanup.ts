 ...
 * 
 * Usage: npx ts-node scripts/utils/aws-resource-cleanup/aws-resource-cleanup.ts
 */
...
interface CleanupResult {
  resourceType: string;
  resourceName: string;
  status: 'deleted' | 'failed' | 'not_found' | 'skipped';
  error?: string;
}
...
class TrinityResourceCleanup {
  ...
  private isTrinityResource(resourceName: string): boolean {
    return TRINITY_PREFIXES.some(prefix => 
      resourceName.toLowerCase().includes(prefix.toLowerCase())
    );
  }

  private addResult(resourceType: string, resourceName: string, status: CleanupResult['status'], error?: string): void {
    this.results.push({
      resourceType,
      resourceName,
      status,
   ...
   */
  async cleanupLambdaFunctions(): Promise<void> {
    ...
    try {
      ...
      for (const func of trinityFunctions) {
        ...
        try {
          ...
          this.addResult('Lambda', func.FunctionName, 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupDynamoDBTables(): Promise<void> {
    ...
    try {
      ...
      for (const tableName of trinityTables) {
        try {
          ...
          this.addResult('DynamoDB', tableName, 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupCloudFormationStacks(): Promise<void> {
    ...
    try {
      ...
      for (const stack of trinityStacks) {
        ...
        try {
          ...
          this.addResult('CloudFormation', stack.StackName, 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupAppSyncAPIs(): Promise<void> {
    ...
    try {
      ...
      for (const api of trinityAPIs) {
        ...
        try {
          ...
          this.addResult('AppSync', api.name || api.apiId || 'unknown', 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupIAMRoles(): Promise<void> {
    ...
    try {
      ...
      for (const role of trinityRoles) {
        ...
        try {
          ...
          this.addResult('IAM Role', role.RoleName, 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupS3Buckets(): Promise<void> {
    ...
    try {
      ...
      for (const bucket of trinityBuckets) {
        ...
        try {
          ...
          this.addResult('S3 Bucket', bucket.Name, 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupAPIGateway(): Promise<void> {
    ...
    try {
      ...
      for (const api of trinityAPIs) {
        ...
        try {
          ...
          this.addResult('API Gateway', api.name || api.id || 'unknown', 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  async cleanupCognitoUserPools(): Promise<void> {
    ...
    try {
      ...
      for (const pool of trinityUserPools) {
        ...
        try {
          ...
          this.addResult('Cognito User Pool', pool.Name || pool.Id || 'unknown', 'failed', error instanceof Error ? error.message : String(error));
        }
      }
    } catch (error) {
   ...
   */
  generateSummary(): CleanupSummary {
...
export { TrinityResourceCleanup, CleanupResult, CleanupSummary };