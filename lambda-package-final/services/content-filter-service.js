...
class ContentFilterService {
    ...
    async generatePrioritizedContent(criteria, excludeIds) {
        ...
        console.log(`🔍 DEBUG: criteria.genres = [${criteria.genres.join(', ')}], length = ${criteria.genres.length}`);
        console.log(`🔍 DEBUG: criteria.mediaType = ${criteria.mediaType}`);
        console.log(`🔍 DEBUG: criteria.roomId = ${criteria.roomId}`);
        
        const results = [];
        const now = new Date();
        try {
            console.log(`🎯 ContentFilterService: Starting content generation for ${criteria.mediaType} with genres [${criteria.genres.join(',')}]`);
            
            // Priority 1: Content with ALL selected genres (AND logic) - up to 30 items
            ...
            if (criteria.genres.length > 0) {
                ...
                for (let page = 1; page <= maxPages; page++) {
                    try {
                        ...
                        console.log(`🥇 Priority 1: Fetching page ${page} with genres ${criteria.genres.join(',')}`);
                        ...
                        console.log(`🥇 Priority 1: Page ${page} returned ${pageContent.length} items`);
                        ...
                        const validatedContent = pageContent.filter(item => {
                            ...
                            if (!hasAllGenres) {
                                console.log(`⚠️ Priority 1: Filtering out ${item.title || item.name} - missing genres. Has: [${itemGenres.join(',')}], Needs: [${criteria.genres.join(',')}]`);
                            }
                            return hasAllGenres;
                        });
                        
                        allGenresContent.push(...validatedContent);
                        console.log(`🥇 Priority 1: Page ${page} - ${validatedContent.length}/${pageContent.length} items passed genre validation`);
                        
                        // Stop if we have enough content for good randomization
                        if (allGenresContent.length >= 50) break;
                    } catch (pageError) {
                        console.error(`❌ Priority 1 Page ${page} error: ${pageError.message}`);
                        break;
                    }
                }
                
                console.log(`🥇 Priority 1: Total collected ${allGenresContent.length} items with ALL genres`);
                ...
                console.log(`✅ Priority 1: Added ${priority1Items.length} items with ALL genres [${criteria.genres.join(',')}]`);
            ...
            // Priority 2: Content with ANY of the selected genres (OR logic) - only if Priority 1 didn't get 30 items
            if (criteria.genres.length > 0 && results.length < 30) {
                console.log(`🥈 Priority 2: Priority 1 only got ${results.length} items, fetching content with ANY genre (OR logic)`);
                const currentExcludeIds = [...excludeIds, ...results.map(r => r.tmdbId)];
                
                // Make separate calls for each genre to implement OR logic
                let anyGenreContent = [];
                const seenIds = new Set();
                
                for (const genreId of criteria.genres) {
                    console.log(`🥈 Priority 2: Fetching content with genre ${genreId}`);
                    ...
                    const filteredContent = genreContent.filter(item => {
                        ...
                        const hasThisGenre = itemGenres.includes(genreId);
                        
                        // Only include if it has this genre but NOT all genres (to avoid Priority 1 duplicates)
                        if (hasThisGenre && !hasAllGenres) {
                            seenIds.add(itemId);
                            return true;
                        }
                        
                        if (!hasThisGenre) {
                            console.log(`⚠️ Priority 2: ${item.title || item.name} doesn't have genre ${genreId}. Has: [${itemGenres.join(',')}]`);
                        }
                    ...
                    console.log(`🥈 Priority 2: Genre ${genreId} added ${filteredContent.length} items`);
                }
                ...
                console.log(`✅ Priority 2: Added ${priority2Items.length} items with ANY genre from ${anyGenreContent.length} OR logic candidates`);
            }
            
            // Priority 3: Popular content - only if we still don't have 30 items
            if (results.length < 30) {
                console.log(`🥉 Priority 3: Still need ${30 - results.length} items, fetching popular ${criteria.mediaType} content`);
                const currentExcludeIds = [...excludeIds, ...results.map(r => r.tmdbId)];
                ...
                console.log(`✅ Priority 3: Added ${priority3Items.length} popular items`);
            }
            ...
            for (const item of results) {
                if (!seenIds.has(item.tmdbId)) {
                    ...
                    console.log(`🚫 Removing duplicate: ${item.title} (${item.tmdbId})`);
                }
            }
            
            console.log(`🎯 ContentFilterService: Generated ${uniqueResults.length} total items for ${criteria.mediaType}`);
            console.log(`📊 Priority distribution: P1=${uniqueResults.filter(r => r.priority === 1).length}, P2=${uniqueResults.filter(r => r.priority === 2).length}, P3=${uniqueResults.filter(r => r.priority === 3).length}`);
            
            // Log sample of results for debugging
            if (uniqueResults.length > 0) {
                console.log(`🔍 Sample results:`);
                uniqueResults.slice(0, 3).forEach((item, index) => {
                    console.log(`   ${index + 1}. ${item.title} (P${item.priority}) - Genres: [${item.genreIds.join(',')}]`);
                });
            }
